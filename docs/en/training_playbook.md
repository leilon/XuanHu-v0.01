...
```bash
python3 trainer/core/train_qlora.py \
...
- script: `trainer/rl/synthesize_agentic_rl_pairs.py`
- idea:
...
`trainer/rl/build_agentic_rl_data.py` additionally builds preference pairs from:
- benchmark seeds
...
```bash
python3 trainer/rl/build_agentic_rl_data.py \
...
```bash
python3 trainer/data/download_medical_datasets.py --root /root/autodl-tmp/medagent/datasets
python3 trainer/data/prepare_medical_training_data.py --root /root/autodl-tmp/medagent/datasets
```
...
```bash
python3 trainer/rl/train_agentic_rl.py \
  --base-model /root/autodl-tmp/medagent/models/qwen2.5-7b-instruct \
