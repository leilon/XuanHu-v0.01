...
2. 再做 query 重写
   - `QR`：`Query Rewriting`
   - 目标是把用户口语化、信息不完整的提问，重写成更适合医学检索的表达
3. 再做 HyDE 扩展
   - `HyDE`：`Hypothetical Document Embeddings`
   - 先生成一段“假设性临床文档/理想检索文档”，让检索更贴近诊断学描述
4. 再做混合检索
   - BM25 稀疏检索
   - dense embedding 检索
5. 对 top-K 结果做 rerank
6. 最终生成时要求带证据引用
...
- 文档下载、HTML/XML 清洗、chunk 切分：主要是 `CPU` 活
- embedding 计算和批量建索引：`CPU` 能做，`GPU` 会更快
- 所以在还没租卡的时候，完全可以先把：
  - 原始文档下载好
  - 结构化清洗好
  - chunk 文件准备好
